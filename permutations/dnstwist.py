...
class DomainFuzz():
	...
	def generate(self):
		self.domains.append({'fuzzer': '', 'domain-name': '.'.join(filter(None, [self.subdomain, self.domain, self.tld]))})
		for domain in self.__addition():
			self.domains.append({'fuzzer': '', 'domain-name': '.'.join(filter(None, [self.subdomain, domain, self.tld]))})
			out.append({'fuzzer': '', 'domain-name': '.'.join(filter(None, [self.subdomain, domain, self.tld]))})
		...
		for tld in self.__tld():
			self.domains.append({'fuzzer': '', 'domain-name': '.'.join(filter(None, [self.subdomain, self.domain, tld]))})
			out.append({'fuzzer': '', 'domain-name': '.'.join(filter(None, [self.subdomain, domain, self.tld]))})
		if '.' in self.tld:
			self.domains.append({'fuzzer': '', 'domain-name': self.domain + '.' + self.tld.split('.')[-1]})
			self.domains.append({'fuzzer': '', 'domain-name': self.domain + self.tld})
			out.append({'fuzzer': '', 'domain-name': self.domain + '.' + self.tld.split('.')[-1]})
			out.append(({'fuzzer': '', 'domain-name': self.domain + self.tld}))
		if '.' not in self.tld:
			self.domains.append({'fuzzer': '', 'domain-name': self.domain + self.tld + '.' + self.tld})
			out.append({'fuzzer': '', 'domain-name': self.domain + self.tld + '.' + self.tld})
		if self.tld != 'com' and '.' not in self.tld:
			self.domains.append({'fuzzer': '', 'domain-name': self.domain + '' + self.tld + '.com'})
			out.append({'fuzzer': '', 'domain-name': self.domain + '' + self.tld + '.com'})
